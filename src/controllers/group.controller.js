...
const postGroup = async (req, res) => {
  ...
  const { name, student_amount, teacher_id, course_id } = req.body;
  ...
  const CreateGr = await Gruop.create({
    name,
    image,
    teacher_id,
    student_amount,
...
const putGroup = async (req, res) => {
  ...
  const { imageName: image } = req;
  const { name, student_amount, teacher_id, course_id } = req.body;
  const editGruop = await Gruop.findByIdAndUpdate(
    id,
    { name, image, teacher_id, student_amount, course_id },
    { new: true }
  );
  if (!editGruop) {
